...
describe('/authors', () => {
    ...
    describe('with no records in the database', ()=> {
      describe('POST /authors', () => {
        it('creates a new author in the database', async () => {
            ...
            expect(response.body.author).to.equal('Junji Ito');

            expect(newAuthorRecord.author).to.equal('Junji Ito');
        });

        it('throws an error if field is empty', async () => {
            const response = await request(app).post('/authors').send({});
            const newAuthorRecord = await Author.findByPk(response.body.id, {
                raw: true,
            });
        ...
        it('throws an error if the field is not valid', async () => {
            const response = await request(app).post('/authors').send({
                author: '',
            });
            const newAuthorRecord = await Author.findByPk(response.body.id, {
    ...
    describe('with records in the database', () => {
      ...
      beforeEach(async () => {
        ...
        Author.create({ author: 'Stpehen King' }),
        Author.create({ author: 'Robert Frost' }),
        ]);
    });
    ...
    describe('PATCH /authors/:id', () => {
      it('updates author by id', async () => {
        ...
        expect(response.status).to.equal(200);
        expect(updatedAuthorRecord.author).to.equal('Stephen King');
        });
